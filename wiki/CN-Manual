...
CCXT 库是许多可用的加密货币交易所，或者说交易所类/class的集合。每个类都为特定的加密货币交易所实现了其公开和私有 API。所有的交易所类都基于 base Exchange 类，并且拥有一些相同的方法/函数。如果您想通过 CCXT 连接某个交易所，您需要创建一个相应交易所的实例。CCXT 已支持的交易所类将会被频繁更新，并且会依据计划添加新交易所的支持。    
...
目前下列所有交易所的公开和私有 HTTP REST APIs 均已被实现。JavaScript, PHP, Python和其他语言的 WebSocket 和 FIX 实现将会尽很快到来。  

- [交易所](#交易所)
- [市场](#市场)
- [API 方法 / Endpoints](#API方法/Endpoints)
- [市场数据](#市场数据)
- [交易](#交易)
...
除了提供基本的市价和限价单之外，一些交易所还提供保证金交易（杠杆），各种衍生工具（如期货合约和期权），还有[黑池]((https://en.wikipedia.org/wiki/Dark_pool))，[OTC ](https://en.wikipedia.org/wiki/Over-the-counter_(finance))(场外交易)，做市 API 等等。
...
PHP 版本的 ccxt 库使用内置的 UTC/GMT 时间函数，因此在使用 PHP 版本之前，您需要在 php.ini 中设置 date.timezone 或调用 [date_default_timezone_set()]((http://php.net/manual/en/function.date-default-timezone-set.php)) 函数。建议将时区设置为 `"UTC"`。
...
每个交易所都有一系列的属性和方法，其中大多数您可以通过向交易所构造函数传递一个包含相关参数的数组来重写。您也可以创建一个子类并覆盖所有内容。

作为例子，以下是一个添加了值的基础交易所的属性的概述：

```JavaScript
{
    'id':   'exchange'                  // lowercase string exchange id 小写的交易所 id 字符串
    'name': 'Exchange'                  // human-readable string 交易所名，易读的字符串
    'countries': [ 'US', 'CN', 'EU' ],  // string or array of ISO country codes ISO 国家代码的字符串或字符串列表
    'urls': {
        'api': 'https://api.example.com/data',  // string or dictionary of base API URLs 包含 base API 链接的字符串或字典
        'www': 'https://www.example.com'        // string website URL 交易所网站链接
        'doc': 'https://docs.example.com/api',  // string URL or array of URLs 交易所文档链接，字符串或者字符串列表
    },
    'version':         'v1',            // string ending with digits 版本号，以数字结尾的字符串
    'api':             { ... },         // dictionary of api endpoints API 端点的字典
    'has': {                            // exchange capabilities 功能列表
        'CORS': false,
        'publicAPI': true,
        'privateAPI': true,
        'cancelOrder': true,
    ...
    'timeframes': {                     // empty if the exchange !has.fetchOHLCV 如果交易所没有 fetchOHLCV 则时间帧为空
        '1m': '1minute',
        '1h': '1hour',
        '1d': '1day',
        '1M': '1month',
    ...
    'timeout':          10000,          // number in milliseconds 超时：以毫秒记的数字
    'rateLimit':        2000,           // number in milliseconds 速率限制：以毫米记的数字
    'userAgent':       'ccxt/1.1.1 ...' // string, HTTP User-Agent header HTTP 请求头信息
    'verbose':          false,          // boolean, output error details 详尽模式：显示详细错误信息
    'markets':         { ... }          // dictionary of markets/pairs by symbol 市场：以 symbol 为键的市场交易对字典
    'symbols':         [ ... ]          // sorted list of string symbols (traded pairs) symbols：有序的 symbol 字符串列表
    'currencies':      { ... }          // dictionary of currencies by currency code 
    'markets_by_id':   { ... },         // dictionary of dictionaries (markets) by id 
    'proxy': 'https://crossorigin.me/', // string URL  代理：URL 字符串
    'apiKey':   '92560ffae9b8a0421...', // string public apiKey (ASCII, hex, Base64, ...)
    'secret':   '9aHjPmW+EtRRKN/Oi...'  // string private secret key
    'password': '6kszf4aci8r',          // string password 密码：字符串（部分交易所有）
    'uid':      '123456',               // string user id uid：用户 id（部分交易所有）
}
```
